...
images_dir = r"C:\Users\bhara\OneDrive\Pictures\images"
pdf_output_path = os.path.join(desktop_dir, "Chamakura_Akhil_Reddy_Technical_Report.pdf")

# Explicit filename mapping to prevent any mix-ups
img_files = {
    'dash': 'dash.png',
    'evt': 'evt.png',
    'det': 'det.png',
...
for key, fname in img_files.items():
    ...
    if os.path.exists(fpath):
        with open(fpath, 'rb') as f:
            encoded_imgs[key] = f"data:image/png;base64,{base64.b64encode(f.read()).decode('utf-8')}"
        print(f"Successfully loaded: {fname}")
    else:
        # Fallback search if exact short name isn't found
        print(f"Warning: {fname} not found in exact form, checking folder...")
        encoded_imgs[key] = ""
...
<body>
    ...
    <div class="meta-box">
        <table class="meta-table">
            ...
            <tr><td><b>Stack:</b></td><td colspan="3">Node.js, Express.js, MongoDB Atlas, EJS, Bootstrap 5</td></tr>
        </table>
    </div>
    <h2>1. Executive Summary & Architecture</h2>
    <p>The <b>Campus Event Manager</b> is a multi-tier platform engineered for managing university events, participant tracking, and real-time operational analytics built using the MVC pattern on Express.js and MongoDB Atlas.</p>
    <table class="data-table">
        <tr><th style="width: 20%;">Layer</th><th style="width: 25%;">Technology</th><th>Implementation Details</th></tr>
        <tr><td>Backend Runtime</td><td>Node.js / Express.js</td><td>Handles REST routing, middleware request validation, dynamic template processing.</td></tr>
        <tr><td>Database Layer</td><td>MongoDB Atlas / Mongoose ODM</td><td>Document persistence store for user entities and embedded event registration arrays.</td></tr>
        <tr><td>Front-End Layer</td><td>EJS & Bootstrap 5</td><td>Server-rendered dynamic templates with responsive desktop/mobile UI components.</td></tr>
        <tr><td>Data Analytics</td><td>MongoDB Aggregations</td><td>Executes multi-stage analytical queries ($unwind, $group, $lookup, $project).</td></tr>
    </table>
    <h2>2. System Interfaces & Real Outputs</h2>
    <h3>2.1 Main Dashboard View</h3>
    <div class="fig-box">
        <img src="{encoded_imgs.get('dash', '')}">
        <div class="caption">Figure 1: Main Platform Dashboard featuring real-time event stats and popular event highlights.</div>
    </div>
    <h3>2.2 Events Directory View</h3>
    <div class="fig-box">
        <img src="{encoded_imgs.get('evt', '')}">
        <div class="caption">Figure 2: Events Catalog displaying filter dropdowns and venue location data.</div>
    </div>
    <h3>2.3 Event Details & Registration Interface</h3>
    <div class="fig-box">
        <img src="{encoded_imgs.get('det', '')}">
        <div class="caption">Figure 3: Event Details page featuring interactive registration triggers and attendee tables.</div>
    </div>
    <h3>2.4 User Directory Interface</h3>
    <div class="fig-box">
        <img src="{encoded_imgs.get('usr', '')}">
        <div class="caption">Figure 4: User Directory table with user role indicators and event history controls.</div>
    </div>
    <h2>3. MongoDB Aggregation Pipelines</h2>
    <h3>3.1 Analytics Dashboard View</h3>
    <div class="fig-box">
        <img src="{encoded_imgs.get('ana', '')}">
        <div class="caption">Figure 5: Analytics UI showing operational data aggregations.</div>
    </div>
    <p><b>Query 1: Total Registrations by Category</b></p>
  ...
  {{ $sort: {{ confirmedRegistrations: -1 }} }}
    ...
    <p><b>Query 2: Top 5 Events by Occupancy Rate</b></p>
    <div class="code-block">db.events.aggregate([
  ...
  {{ $limit: 5 }}
    ...
    <p><b>Query 3: Inactive Users Search</b></p>
  ...
  {{ $project: {{ firstName: 1, lastName: 1, email: 1, role: 1 }} }}
    ...
    <h2>4. Submission Verification</h2>
    <table class="data-table">
        <tr><td style="width: 30%;"><b>Student Author</b></td><td>Chamakura Akhil Reddy</td></tr>
        <tr><td><b>Project Title</b></td><td>Campus Event Manager</td></tr>
        <tr><td><b>GitHub Repository</b></td><td>https://github.com/akhill-reddy/campus-event-manager</td></tr>
    </table>
</body>
</html>
"""

html_temp_path = os.path.join(project_dir, "report_render.html")
with open(html_temp_path, "w", encoding="utf-8") as f:
    f.write(html_content)

print("\nRendering clean high-res PDF via Playwright...")
with sync_playwright() as p:
    browser = p.chromium.launch()
    page = browser.new_page()
    page.goto(f"file:///{html_temp_path.replace('\\', '/')}")
    page.pdf(
